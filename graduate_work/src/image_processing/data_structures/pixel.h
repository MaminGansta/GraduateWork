...
struct Pixel
{
    ...
    float Distance(const Pixel& other, MeanShitParams params) const
    {
        float res = 0;
        float brightness1 = 0;
        float brightness2 = 0;

        res += std::pow(x - other.x, 2) * params.DistanceCoef;
        res += std::pow(y - other.y, 2) * params.DistanceCoef;

        for (int i = 0; i < 3; i++)
        {
            res += std::pow(color[i] - other.color[i], 2) * params.ColorCoef;
            //brightness1 += color[i];
